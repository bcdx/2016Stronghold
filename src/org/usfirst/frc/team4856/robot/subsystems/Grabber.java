...
// Though inheritance, DriveTrain inherits all the traits of the class Subsystem, and will have any new traits we assign to it.
  ...
  //when no other command is runnin.g, the default command is tankdrivewithjoystick. Consult the command TankDriveWithJoystick for more info.
    
        // Set the default command for a subsystem here.
        //setDefaultCommand(new MySpecialCommand());
    }
    
    public double speedShooter = 0;
    public double speedChange = 0.1;
	...
	public void increase() {
		if (speedShooter < 1) {
			speedShooter=speedShooter+speedChange;
		    grabberMotor.set(speedShooter);
		    //increase the speed by speedChange
		}

	} 
	
	public void decrease() {
		if (speedShooter >= speedChange) {
			speedShooter=speedShooter-speedChange;
			grabberMotor.set(speedShooter);
	    //decrease the speed by speedChange
		}
	}
